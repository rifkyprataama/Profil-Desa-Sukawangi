    ...
    fetch(`${API_URL}/api/kontak`)
      ...
      .then(data => {
        if (data.success && data.data) {
          setDataKontak(Array.isArray(data.data) ? data.data[0] : data.data);
        }
      })
      .catch(err => console.error("Gagal mengambil data kontak untuk footer:", err));
  }, [API_URL]);
      ...
      <footer className="bg-[#012d1d] text-white pt-16 pb-8 mt-auto border-t-[6px] border-[#febe9b] relative overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10">
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* LINK SOSIAL MEDIA DINAMIS (Muncul hanya jika diisi oleh Admin) */}
            {dataKontak?.link_instagram && (
              <a href={dataKontak.link_instagram} target="_blank" rel="noreferrer" className="text-white/80 hover:text-white hover:translate-x-1 transition-all text-[15px] flex items-center gap-2">
                <ChevronRight className="w-4 h-4"/> Instagram Resmi
              </a>
            )}
            
            {dataKontak?.link_facebook && (
              <a href={dataKontak.link_facebook} target="_blank" rel="noreferrer" className="text-white/80 hover:text-white hover:translate-x-1 transition-all text-[15px] flex items-center gap-2">
                <ChevronRight className="w-4 h-4"/> Halaman Facebook
              </a>
            )}
            
            {dataKontak?.link_youtube && (
              <a href={dataKontak.link_youtube} target="_blank" rel="noreferrer" className="text-white/80 hover:text-white hover:translate-x-1 transition-all text-[15px] flex items-center gap-2">
                <ChevronRight className="w-4 h-4"/> Channel YouTube
              </a>
            )}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="text-white/80 text-[14px] flex items-start gap-3">
              ...
              <p className="leading-relaxed">{dataKontak?.alamat || 'Alamat belum diatur'}</p>
            </div>
            <div className="text-white/80 text-[14px] flex items-center gap-3">
              <Mail className="w-5 h-5 shrink-0 text-[#febe9b]" />
              <p>{dataKontak?.email || 'Email belum diatur'}</p>
            </div>
            <div className="text-white/80 text-[14px] flex items-center gap-3">
              <PhoneCall className="w-5 h-5 shrink-0 text-[#febe9b]" />
              <p>{dataKontak?.jam_operasional || 'Jam belum diatur'}</p>
            </div>
          </div>
        ...
        <div className="max-w-[1200px] mx-auto px-6 mt-12 pt-6 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 text-white/60 text-[13px] relative z-10">
          <p>© {tahunSekarang} Pemerintah Desa Sukawangi. Seluruh Hak Cipta Dilindungi.</p>
